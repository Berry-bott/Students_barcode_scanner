...
const BarcodeScanner = () => {
  ...
  const handleScanSuccess = async (decodedText) => {
    if (hasScannedRef.current) return; // ⛔ Skip duplicate scan
  ...
  useEffect(() => {
    if (scanResult) {
      const timer = setTimeout(() => {
        setScanResult(null);
        hasScannedRef.current = false; // 🔄 Reset scan lock after modal disappears
      }, 3000);
  ...
  const startScanner = () => {
    ...
    html5QrCodeRef.current
      ...
      .then(() => {
        setIsScanning(true);
        hasScannedRef.current = false; // ✅ Reset before each new scan session
      })
      .catch((err) => {
